...
# Make a bar graph
ggplot(avg_price_by_store, aes(y = Store, x = avg_price)) +
...
# Make a bar graph
ggplot(num_stores_per_product, aes(x = Product, y = num_stores)) +
  geom_bar(stat = "identity") +
...
# Get coefficients
summary(model_C)
# Coefficient Estimate for TypeConvenience
coef_estimate <- -0.66426

# Standard Error
std_error <- 0.12765

# Critical value (z-value) for 95% confidence interval
critical_value <- 1.96

# Calculate confidence interval
lower_bound <- coef_estimate - critical_value * std_error
upper_bound <- coef_estimate + critical_value * std_error
...
# Print the results
cat("The 95% confidence interval for the price difference between convenience stores and ordinary grocery stores is approximately between $", lower_bound, "and $", upper_bound, ".\n")
